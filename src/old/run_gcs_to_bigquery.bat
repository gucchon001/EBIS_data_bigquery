...
:: GCSからBigQueryへのデータロード実行スクリプト
echo GCSからBigQueryへのデータロード処理を実行します...
...
if "%GCS_PATH%"=="" (
    echo AE_SSresultディレクトリのデータをBigQueryにロードします...
    %PYTHON_CMD% src/modules/bigquery/load_ae_ssresult.py
    if %ERRORLEVEL% neq 0 (
    ...
    echo 指定されたGCSパスからBigQueryにデータをロードします...
    ...
    %PYTHON_CMD% src/modules/bigquery/gcs_to_bigquery.py !ARGS!
